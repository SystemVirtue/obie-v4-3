...
export const ApiKeyTestDialog: React.FC<ApiKeyTestDialogProps> = ({
  ...
  useEffect(() => {
    if (isOpen && !hasStarted) {
      console.log("[ApiKeyTestDialog] Starting immediate API key tests...");
      setHasStarted(true);
      startTesting();
    }
  }, [isOpen, hasStarted]);
  ...
  const startTesting = async () => {
    ...
    // Test each key sequentially with NO delays
    for (let i = 0; i < API_KEYS.length; i++) {
      ...
      try {
        ...
        const isSuccess = quotaUsage.percentage < 90;
        const newStatus: ApiKeyTestResult["status"] = isSuccess ? "success" : "quota_exceeded";
        ...
        // IMMEDIATELY use first valid key found
        if (isSuccess) {
          console.log(`[ApiKeyTestDialog] Found valid key immediately: ${apiKey.name}`);
          setSelectedKey(apiKey.name);
          setIsComplete(true);
          
          // Build results with this successful key
          const finalResults = initialResults.map((r, idx) => {
            if (idx === i) {
              return { ...r, status: newStatus, quotaUsage };
            }
            return r;
          });
          
          // Call completion handler immediately - no countdown
    ...
    // All keys tested - complete without countdown
    setIsComplete(true);
    
    // Build final results
    setTestResults((currentResults) => {
      const workingIndex = currentResults.findIndex((r) => r.status === "success");
      if (workingIndex >= 0) {
  ...
  const testApiKey = async (index: number) => {
    // This function is no longer used - logic moved to startTesting for immediate detection
  };
