...
export const useSerialCommunication = ({
  ...
  const setupSerialConnection = async () => {
    try {
      ...
      // Auto-connect if USB Serial-1420 is found
      if (!targetPort) {
        try {
          console.log('No previous connection found, scanning for USB Serial-1420...');
          const availablePorts = await (navigator as any).serial.getPorts();
          
          // Try to find USB Serial-1420 device
          for (const port of availablePorts) {
            const info = port.getInfo();
            console.log('Found serial device:', info);
            if (info.usbProductId === 1420 || info.usbVendorId === 1420) {
              targetPort = port;
              console.log('Auto-detected USB Serial-1420, connecting...');
              break;
            }
          }
          
          // If not found in existing ports, request selection only once
          if (!targetPort && !hasCancelledPortSelection.current) {
            console.log('USB Serial-1420 not found, requesting port selection...');
            targetPort = await (navigator as any).serial.requestPort();
          }
        } catch (portError) {
          console.error('Port selection error:', portError);
          setConnectionStatus('error');
          toast({
        ...
        console.log('Found previously connected USB Serial-1420, auto-connecting...');
      }
